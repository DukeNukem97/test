...
public class NotificationService {
    ...
    public void addTestDatas(){
        ...
        Notifications notifications2=new Notifications(2,"Message2",false,new java.util.Date());
    ...
    public ResponseEntity<?> updateNotification(int userId,Notifications notifications){
        User user=userRepository
                .findById(userId).
                orElseThrow(() -> new ResourceNotFoundException("User not found with id: "+userId));
        List<Notifications> updatedNotification=new ArrayList<>();
        Iterator<Notifications> iterator=user.getNotifications().iterator();
        while(iterator.hasNext()){
            Notifications notify=iterator.next();
            if(notify.getNotificationId()==notifications.getNotificationId()){
                notify.setRead(true);
            }
            updatedNotification.add(notify);
        }
        user.setNotifications(updatedNotification);
        this.userRepository.save(user);
